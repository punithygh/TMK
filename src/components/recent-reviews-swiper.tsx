    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-6 px-1">
        <h2 className="text-xl md:text-2xl font-bold text-white flex items-center gap-2">
          <MessageSquareQuote className="text-sky-400" size={24} />
      ...
      <div className="flex gap-4 overflow-x-auto pb-6 scrollbar-hide snap-x snap-mandatory px-1 -mx-1">
        ...
        {initialReviews.map((review) => {
            ...
            return (
              <div 
                key={review.id} 
                className="min-w-[280px] md:min-w-[320px] max-w-[320px] flex-shrink-0 bg-[#0c1220] border border-slate-800 rounded-2xl p-5 snap-center md:snap-start shadow-lg hover:border-sky-500/30 transition-all duration-300 flex flex-col h-full"
              >
                {/* User Info & Rating */}
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-sky-500/10 text-sky-500 flex items-center justify-center font-bold text-lg border border-sky-500/20">
                ...
                <Link 
                  ...
                  className="mt-auto flex items-center gap-2 bg-slate-900 border border-slate-700 hover:border-sky-500 hover:bg-slate-800 p-2.5 rounded-xl transition-all group no-underline"
                >
                  ...
                  <div className="flex flex-col overflow-hidden">
                    ...
                    <span className="text-white font-bold text-xs truncate group-hover:text-sky-400 transition-colors">
                      {bizName}
                    </span>
                  </div>
                </Link>
