...
import { getOneCourse, BusinessListing } from "@/services/courses";
...
export default function BusinessDetailPage({ params }: Props) {
  ...
  const [isNotFound, setIsNotFound] = useState(false); // 🚨 ಹೊಸದು: 404 ಕ್ರಾಶ್ ತಡೆಯಲು
  ...
  // 🚨 1. ಡೇಟಾ ಫೆಚ್ ಮತ್ತು ಸೇಫ್ ಎರರ್ ಹ್ಯಾಂಡ್ಲಿಂಗ್
  useEffect(() => {
    const fetchBusiness = async () => {
      ...
      // ಡೇಟಾ ಸಿಗದಿದ್ದರೆ (Django 404 ಕೊಟ್ಟರೆ)
      if (!data) {
        setIsNotFound(true);
        setIsLoading(false);
        return;
  ...
  // 🚨 Generate dynamic highlights from backend data
  const getHighlights = () => {
    if (!business) return [];
    const highlights = [];
    ...
    // Add generic highlights if needed
  ...
  const categorizedServices = useMemo(() => {
    ...
    // Build services list from backend flags
    const rawServices = [];
    ...
    // Add fallback generic services if not enough
  ...
  if (isLoading) {
    return (
      <div className="w-full min-h-[70vh] flex flex-col items-center justify-center bg-[#050b14]">
        <Loader2 className="w-10 h-10 text-sky-500 animate-spin mb-4" />
        <p className="text-slate-400 font-medium">{t("ಮಾಹಿತಿ ಪಡೆಯಲಾಗುತ್ತಿದೆ...", "Loading details...")}</p>
      </div>
    );
  }

  // 🚨 3. ಪ್ರೀಮಿಯಂ ಇನ್-ಪೇಜ್ 404 ಸ್ಟೇಟ್ (ಕ್ರಾಶ್ ಆಗುವುದನ್ನು ತಡೆಯುತ್ತದೆ)
  if (isNotFound || !business) {
    return (
      <div className="w-full min-h-[75vh] flex flex-col items-center justify-center bg-[#050b14] px-4 text-center">
        <div className="w-24 h-24 bg-slate-900 rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(14,165,233,0.15)]">
          <SearchX className="w-12 h-12 text-sky-500" />
        </div>
        <h1 className="text-3xl md:text-4xl font-black text-white mb-3">
          {t("ಬ್ಯುಸಿನೆಸ್ ಕಂಡುಬಂದಿಲ್ಲ", "Business Not Found")}
        </h1>
        <p className="text-slate-400 max-w-md mx-auto mb-8 leading-relaxed">
          {t(
            "ನೀವು ಹುಡುಕುತ್ತಿರುವ ಬ್ಯುಸಿನೆಸ್ ನಮ್ಮಲ್ಲಿ ಇಲ್ಲದಿರಬಹುದು ಅಥವಾ ಲಿಂಕ್ ತಪ್ಪಾಗಿರಬಹುದು.", 
            "The business you are looking for might have been removed or the link is incorrect."
          )}
        </p>
        <Link href="/listings" className="bg-sky-500 hover:bg-sky-400 text-white font-bold py-3 px-8 rounded-xl transition-all shadow-lg shadow-sky-500/20 hover:-translate-y-1">
          {t("ಎಲ್ಲಾ ಬ್ಯುಸಿನೆಸ್‌ಗಳನ್ನು ನೋಡಿ", "Browse All Businesses")}
        </Link>
      </div>
    );
  }
    ...
    <div className="w-full bg-[#050b14] min-h-screen pb-24 md:pb-10 font-poppins">
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-[3%] pt-6">
        ...
        <div className="flex md:hidden justify-center gap-1.5 mb-5">
          <span className={`h-1.5 rounded-full transition-all ${activeGalleryDot === 0 ? 'w-4 bg-sky-500' : 'w-1.5 bg-slate-700'}`}></span>
          <span className={`h-1.5 rounded-full transition-all ${activeGalleryDot === 1 ? 'w-4 bg-sky-500' : 'w-1.5 bg-slate-700'}`}></span>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
          <div className="w-full md:w-auto">
            ...
            <div className="flex items-center gap-3 mt-3">
              ...
              <span className="text-slate-400 text-xs underline cursor-pointer hover:text-white transition-colors">120 Ratings</span>
            </div>
          ...
          <div className="flex gap-3 w-full md:w-auto mt-4 md:mt-0">
            <button onClick={() => setIsSuggestOpen(true)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 border border-slate-700 text-slate-300 rounded-lg hover:border-sky-500 hover:text-sky-400 transition-colors text-sm font-semibold">
              <Edit3 size={16} /> <span className="hidden md:inline">Suggest Edit</span>
            </button>
            <button onClick={() => setIsBookmarked(!isBookmarked)} className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 border rounded-lg transition-colors text-sm font-semibold ${isBookmarked ? 'bg-red-500 border-red-500 text-white' : 'border-red-500 text-red-500 hover:bg-red-500 hover:text-white'}`}>
        ...
        <div className="flex flex-wrap md:flex-nowrap gap-3 mb-8 w-full border-y border-slate-800 py-4">
          {business.phone ? (
            <>
              ...
              <a href={`https://wa.me/91${business.phone}`} target="_blank" className="flex-1 md:flex-none bg-transparent border border-emerald-500 text-emerald-500 hover:bg-emerald-500 hover:text-white flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-transform hover:-translate-y-1">
          ...
          <div className="hidden md:flex gap-3 ml-auto">
            <button className="border border-purple-500 text-purple-500 hover:bg-purple-500 hover:text-white flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm transition-colors">
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          <div className="flex-1 min-w-0">
            ...
            <section id="services" className="mb-10 pb-8 border-b border-slate-800 scroll-mt-[150px]">
              ...
              {/* Show special flags from backend */}
              ...
              <button onClick={() => setIsServicesOpen(true)} className="px-6 py-2.5 bg-transparent border border-sky-500 text-sky-400 rounded-lg text-sm font-semibold hover:bg-sky-500 hover:text-white transition-colors">
                {t("ಎಲ್ಲಾ ಸೇವೆಗಳನ್ನು ನೋಡಿ", "View All Services & Info")}
              </button>
            </section>
          </div>
          ...
          <div className="hidden lg:block w-[320px] shrink-0 sticky top-[150px] h-fit">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl">
              ...
              <form onSubmit={submitForm} className="flex flex-col gap-4">
                <input type="text" placeholder="Name" className="w-full bg-slate-950 border border-slate-700 text-white px-4 py-3 rounded-lg text-sm outline-none focus:border-sky-500" required />
                <input type="tel" placeholder="Mobile Number" className="w-full bg-slate-950 border border-slate-700 text-white px-4 py-3 rounded-lg text-sm outline-none focus:border-sky-500" required />
                <button type="submit" disabled={formStatus !== "idle"} className="w-full bg-sky-500 text-white font-bold py-3 rounded-lg text-sm hover:bg-sky-400 transition-colors flex items-center justify-center gap-2 mt-2">
                  {formStatus === "idle" ? t("ಅತ್ಯುತ್ತಮ ಡೀಲ್ ಪಡೆಯಿರಿ", "Get Best Deal") : <Loader2 className="animate-spin" size={18} />}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={submitForm} className="p-6 flex flex-col gap-4">
              <div className="flex items-center bg-slate-950 border border-slate-700 rounded-lg overflow-hidden focus-within:border-sky-500 h-12">
                ...
                <input type="text" className="w-full bg-transparent text-white text-sm outline-none" placeholder="Your Name" required />
              </div>
              <div className="flex items-center bg-slate-950 border border-slate-700 rounded-lg overflow-hidden focus-within:border-sky-500 h-12">
                <span className="px-4 text-slate-500"><Smartphone size={16} /></span>
                <input type="tel" className="w-full bg-transparent text-white text-sm outline-none" placeholder="Mobile Number" required />
              </div>
